...
class MemoryFlowDecoder(ContentFlowAttentionDecoder):
  def __init__(self, config, embedlayer):
    super().__init__(config, embedlayer)
    ...
    self.logit = nn.Linear(self.config.hidden_size, self.config.num_words)
    



  def forward(self, inputs, enc_outs, src_mask_2, trg_mask, flow_edges, return_attn=False):
    '''
    Args:
      inputs: (batch, dec_seq_len)
      enc_globals: (batch, hidden_size)
    ...
    Returns:
      logits: (batch*seq_len, num_words)
    '''
    src_1 = enc_outs['video_encode_mi']
    src_mask_1 = enc_outs['src_mask_1']
    src_2 = enc_outs['attn_fts_mi']
    src_11 = enc_outs['video_encode']
    src_22 = enc_outs['attn_fts']
    # pdb.set_trace()
    trans_outputs, attn = self.decoder(inputs, src_1, src_11, src_mask_1, src_2, src_22, src_mask_2, trg_mask[:,:-1,:-1], flow_edges)
    logits = self.logit(trans_outputs)
    logits = logits.view(-1, self.config.num_words)
    ...
    #   # memory_keys, memory_values = self.gen_memory_key_value(enc_memories)
    #   memory_keys, memory_values = self.gen_memory_key_value(enc_memories, enc_memories_mi)
    #   vid_memory_keys, vid_memory_values = self.gen_memory_key_value(video_encode, video_encode_mi, vid=True)
    ...
    #   address_params2 = self.address_layer2(torch.cat([h_attn_lstm, prev_attn_memory, torch.mean(enc_memories, 1)], 1))
    ...
    #     memory_keys, memory_values, enc_masks)
    #   vid_attn_score, vid_attn_memory = self.attn(h_attn_lstm, 
    #     vid_memory_keys, vid_memory_values, vid_mask)
    ...
    #     for x in [prev_attn_score, flow_attn_score_1, flow_attn_score_2, flow_attn_score_3]], 2)
    ...
    #   attn_score = interpolate_gate[:,0].unsqueeze(1) * content_attn_score + interpolate_gate[:,1].unsqueeze(1) * flow_attn_score

    #   # final attention
    #   step_attns.append(attn_score)
    #   prev_attn_score = attn_score
    #   # attn_memory = torch.sum(attn_score.unsqueeze(2) * memory_values, 1)
    #   attn_memory = torch.sum(attn_score.unsqueeze(2) * memory_values, 1) + interpolate_gate[:,2].unsqueeze(1) * torch.sum(vid_attn_score.unsqueeze(2) * vid_memory_values, 1)
  ...
  def step_fn(self, words, step, **kwargs):
    states = kwargs['states']
    enc_globals = kwargs['enc_outs']['init_states']
    enc_memories = kwargs['enc_outs']['attn_fts']
    enc_memories_mi = kwargs['enc_outs']['attn_fts_mi']
    video_encode = kwargs['enc_outs']['video_encode']
    video_encode_mi = kwargs['enc_outs']['video_encode_mi']
    vid_mask = kwargs['enc_outs']['vid_mask']
  ...
  def sample_decode(self, inputs, enc_outs, src_mask_2, flow_edges, greedy=True):
    ...
    seqLogprobs = torch.zeros(inputs.size(0), self.config.max_words_in_sent).cuda()

    for i in range(1, self.config.max_words_in_sent):
      # trg_mask = self.nopeak_mask(i)
      trg_mask = self.pred_masks(outputs)
      # pdb.set_trace()
      word, attn = self.decoder(outputs, src_1, src_11, src_mask_1, src_2, src_22, src_mask_2, trg_mask, flow_edges)
      out = self.logit(word)
    ...
    # seq_words, seq_word_logprobs = caption.utils.inference.sample_decode(
    #   words, self.step_fn, self.config.max_words_in_sent, 
    #   greedy=greedy, states=states, enc_outs=enc_outs, memory_masks=enc_masks,
    #   prev_attn_score=prev_attn_score, flow_edges=flow_edges)

    # return seq_words, seq_word_logprobs

  def beam_search_decode(self, words, enc_outs, enc_masks, flow_edges):
    batch_size, max_attn_len = enc_masks.size()
    device = enc_masks.device
    ...
    sent_pool = caption.utils.inference.beam_search_decode(words, self.step_fn, 
      ...
      sent_pool_size=self.config.sent_pool_size, 
      expand_fn=self.expand_fn, select_fn=self.select_fn,
      enc_outs=enc_outs, memory_masks=enc_masks,
      states=states,
      prev_attn_score=prev_attn_score, flow_edges=flow_edges)
