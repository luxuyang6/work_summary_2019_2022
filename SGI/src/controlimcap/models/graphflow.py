...
class GraphBUTDCFlowAttnModel(controlimcap.models.graphattn.GraphBUTDAttnModel):
  ...
  def forward_loss(self, batch_data, step=None):
    ...
    src_mask_1, trg_mask = self.create_masks(input_batch['vid_len'], input_batch['mp_fts'].size(1),input_batch['caption_ids'])
    src_mask_2 = self.attn_mask(input_batch['attn_len'], max_len=input_batch['attn_fts'].size(1)).unsqueeze(-2)
    enc_outs['src_mask_1'] = src_mask_1

    logits = self.submods[DECODER](input_batch['caption_ids'][:, :-1], 
      enc_outs, src_mask_2, trg_mask,
  ...
  def validate_batch(self, batch_data, addition_outs=None):
    ...
    batch_size = input_batch['mp_fts'].size(0)
    init_words = torch.zeros([batch_size, 1] , dtype=torch.int64).to(self.device)

    src_mask_1 = self.attn_mask(input_batch['vid_len'], max_len=input_batch['mp_fts'].size(1)).unsqueeze(-2)
    src_mask_2 = self.attn_mask(input_batch['attn_len'], max_len=input_batch['attn_fts'].size(1)).unsqueeze(-2)
    enc_outs['src_mask_1'] = src_mask_1
...
class RoleGraphBUTDCFlowAttnModel(GraphBUTDCFlowAttnModel):
  ...
  def forward_encoder(self, input_batch):
    ...
    video_encode= self.submods[VIDEOENC](input_batch['mp_fts'],input_batch['vid_len'])
    # pdb.set_trace()
    ...
    return {'attn_fts': attn_embeds, 'video_encode':video_encode, 'attn_fts_mi': attn_fts_mi, 'video_encode_mi':video_encode_mi, 'vid_mask':input_batch['vid_mask']}
