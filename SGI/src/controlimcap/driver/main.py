...
#       is_train=False, return_label=False, _logger=None)
    ...
    model_files = {}
    if opts.test_all:
      model_file_list = glob.glob(os.path.join(path_cfg.model_dir, 'epoch.*.th'))
      for model_file in model_file_list:
        model_files[model_file.split("/")[-1][:-3]] = model_file
    else:
    ...
    for measure_name, model_file in model_files.items():
      ...
      if not opts.no_evaluate:
        ...
        test_captions = json.load(open('/home/xylu/PHVM/video_asg2cap/ActivityNet/Anet_test_graph.json'))
        ref_captions = {}
        v_dict = {}
