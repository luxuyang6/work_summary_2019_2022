...
def sg_sparse_collate_fn(data):
  outs = {}
  for key in ['names', 'mp_fts', 'vid_len', 'attn_fts', 'attn_len', 'vid_mask', 'attn_masks', 'node_types', 'attr_order_idxs', \
...
class ImageSceneGraphFlowReader(ImageSceneGraphReader):
  def __getitem__(self, idx):    
    ...
    attn_fts, attn_mask , attn_len= self.pad_or_trim_feature(node_name_ids, self.max_attn_len)
