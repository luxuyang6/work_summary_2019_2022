...
data_path = '/home/xylu/PHVM/video_asg2cap/ActivityNet/'
# video_path = '/home/xylu/video_captioning/PHVM/charades-algorithms/Charades_feature/'
video_path = '/home/shares/VideoCaption/Activitynet-Captions/training/'
# data process
if mode == 'train':
    train_data_o = json.load(open(data_path+'Anet_rel_classifier_train.json'))
    data = []
    for d in train_data_o:
        if d[1] == 1:
    ...
    data = json.load(open(data_path+'Anet_rel_classifier_' +dataset+'.json'))
...
for vid in vids:
    ...
    video_fts[vid] = np.load(os.path.join(video_path+vid[2:]+'_resnet.npy'))
    ...
    # for i, test in enumerate(data_p):
        ...
        for j, test in enumerate(batch):
            ...
            if result_dict.get(vid) == None:       
                pred_dict[vid] = []     
                result_dict[vid] = []
            result_dict[vid].append(('_'.join(test[0]), out[j,1].data.tolist()))
            if test[2] == 1:
                pred_dict[vid].append('_'.join(test[0]))
        # if eval_mode == '1':
        #     pred = out.max(1)[1]
