...
class RoleGraphBUTDMemoryFlowModel_old(controlimcap.models.graphflow.RoleGraphBUTDCFlowAttnModel):
  def build_submods(self):
    submods = {}
    submods[MPENCODER] = caption.encoders.vanilla.Encoder(self.config.subcfgs[MPENCODER])
    submods[ATTNENCODER] = controlimcap.encoders.gcn.RoleRGCNEncoder(self.config.subcfgs[ATTNENCODER])
...
class RoleGraphBUTDMemoryFlowModel(controlimcap.models.graphflow.RoleGraphBUTDCFlowAttnModel):
  def build_submods(self):
    submods = {}
    submods[EMBED] = caption.encoders.vanilla.EmbedLayer(self.config.subcfgs[DECODER])
    submods[VIDEOENC] = caption.encoders.vanilla.FramesEncoder()
    submods[MPENCODER] = caption.encoders.vanilla.Encoder(self.config.subcfgs[MPENCODER])
    submods[ATTNENCODER] = controlimcap.encoders.gcn.RoleRGCNEncoder(self.config.subcfgs[ATTNENCODER])
    submods[DECODER] = controlimcap.decoders.memory.MemoryFlowDecoder(self.config.subcfgs[DECODER], submods[EMBED])
    return submods
