...
class ContentFlowAttentionDecoder(caption.decoders.attention.BUTDAttnDecoder):
  def __init__(self, config,embedlayer):
    ...
    self.address_layer = nn.Sequential(
      ...
      nn.ReLU(),
      nn.Linear(memory_size, 3))

    self.address_layer2 = nn.Sequential(
      nn.Linear(self.config.hidden_size*2 + memory_size, memory_size),
      nn.ReLU(),
      nn.Linear(memory_size, 4))
  ...
  def forward(self, inputs, enc_outs, enc_masks, flow_edges, return_attn=False):
    ...
    vid_memory_keys, vid_memory_values = self.gen_memory_key_value(video_encode, video_encode_mi)
    ...
    for t in range(inputs.size(1)):
      ...
      address_params2 = self.address_layer2(torch.cat([h_attn_lstm, prev_memory, torch.mean(enc_memories, 1)], 1))
      interpolate_gate = torch.softmax(address_params)
      ...
      vid_attn_score, video_attn_memory = self.vid_attn(h_attn_lstm, 
      ...
      attn_score = interpolate_gate[:,0] * content_attn_score + interpolate_gate[:,1] * flow_attn_score

      # final attention
      step_attns.append(attn_score)
      prev_attn_score = attn_score
      attn_memory = torch.sum(attn_score.unsqueeze(2) * memory_values, 1) + torch.sum(vid_attn_score.unsqueeze(2) * vid_memory_values, 1)
