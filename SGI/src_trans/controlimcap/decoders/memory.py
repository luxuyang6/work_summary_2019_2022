...
class MemoryFlowDecoder(ContentFlowAttentionDecoder):
  ...
  def forward(self, inputs, enc_outs, enc_masks, flow_edges, return_attn=False):
    '''
    Args:
      inputs: (batch, dec_seq_len)
      enc_globals: (batch, hidden_size)
    ...
    enc_memories_mi = enc_outs['attn_fts_mi']
    video_encode = enc_outs['video_encode']
    video_encode_mi = enc_outs['video_encode_mi']
    vid_mask = enc_outs['vid_mask']
  ...
  def sample_decode(self, words, enc_outs, enc_masks, flow_edges, greedy=True):
    batch_size, max_attn_len = enc_masks.size()
