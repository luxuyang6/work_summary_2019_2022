...
def prepare_attention(mtype):
  ...
  if 'rgcn' in mtype:
    output_dir = '%s%s'%(output_dir, '.embed_first_mi_anet_n' if attn_enc_cfg.embed_first else '')
  print(output_dir)
  ...
  for setname in ['trn', 'val', 'tst']:
    path_cfg['anno_file'][setname] = os.path.join(ROOT_DIR, 'Anet_%s_graph_n.json'%setdict[setname])
