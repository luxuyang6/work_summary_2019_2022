...
ROOT_DIR = '/home/xylu/PHVM/video_asg2cap/Charades'
...
def prepare_attention(mtype):
  ...
  if 'rgcn' in mtype:
    output_dir = '%s%s'%(output_dir, '.embed_first_gt_mi' if attn_enc_cfg.embed_first else '')
  print(output_dir)
  ...
  path_cfg = {
    ...
    'video_dir' : '/home/xylu/PHVM/PHVM_lxy/data/',
  ...
  for setname in ['trn', 'val', 'tst']:
    path_cfg['anno_file'][setname] = os.path.join(ROOT_DIR, 'Charades_v1_%s_graph_b.json'%setdict[setname])
