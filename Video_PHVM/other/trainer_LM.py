...
import data
from utils import *
from models.PHVM_GRU_only_LM import *
...
class Trainer(object):
    ...
    def train_model(self):
        ...
        for step in range(0,self.train_data.num_steps): 
            ...
            loss = self.model(frames, frame_lens, keywords, keyword_lens, texts, texts_lens, segments, group_count, group_lens, captions, caption_lens)
            #loss = stop_loss  + group_loss + word_loss + global_KL + local_KL + bow_loss
            #loss = stop_loss  + group_loss + global_KL + local_KL + bow_loss
            #loss = word_loss
    ...
    def test(self, mode):
        ...
        f_lxy = open("./predictedCaptions.txt", 'w+') #lxy output 
        for i in range(num_steps):
            ...
            predicted_targets = self.model.beam_search(frames, frame_lens, keywords, keyword_lens, segments, group_count)
