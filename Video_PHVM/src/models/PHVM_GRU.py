...
class PHVM(nn.Module):
    ...
    def beam_search(self, frames, frame_lens, keywords, keyword_lens, segments_gt, group_count_gt):
        ...
        #plans, output_ids = self.Decoder.beam_search(keywords, keywords_encode, video_features, frame_lens, segments, group_count,  group_init_state, self.config.PHVM_beam_width)
        plans, output_ids = self.Decoder.beam_search(keywords, keywords_encode, video_features, frame_lens, segments_p, group_count_p, \
                                                     segments_gt, group_count_gt,  group_init_state, self.config.PHVM_beam_width)
...
class GroupEncoder(nn.Module):
    ...
    def forward(self, enc_states, segments, group_count):
        ...
        if batch_size > 1:
            # Sort by length (keep idx)
            group_count, idx_sort = np.sort(group_count)[::-1], np.asarray(np.argsort(-group_count))
            if self.enable_cuda:
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.use_gt_plan = config.use_gt_plan
        ...
        self.initLinear = nn.Linear(self.encoder_dim*2+self.video_hid*2+self.plan_latent_dim+self.PHVM_group_encoder_dim, self.latent_decoder_dim)
    ...
    def beam_search(self, keywords, enc_states, video_features, frame_lens, segments_p, group_count_p, \
        ...
        plans = self._agg_group_plan(group_count_p,segments_p, keywords)
