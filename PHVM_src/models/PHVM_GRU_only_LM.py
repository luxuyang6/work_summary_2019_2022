...
class PHVM(nn.Module):
    ...
    def forward(self, frames, frame_lens, keywords, keyword_lens, texts, texts_lens, segments, group_count, group_lens, captions, caption_lens):
        ...
        word_loss = self.Decoder(captions, caption_lens)
...
class Decoder(nn.Module):
    ...
    def forward(self, captions, caption_lens):
        ...
        for i in range(step_size):
            ...
            word_state = Variable(torch.zeros(2, batch_size, self.decoder_dim)).cuda() 
            output = []
            for k in range(max_caption_len):
                #c_t, _ = self.atten(word_hidden_output, video_features, context_mask)
                #word_inp = torch.cat((embeddings[:,i,k,:], c_t), 1).unsqueeze(1)
            ...
            # Sort by length (keep idx)
                ...
                caption_len, idx_sort = np.sort(caption_len)[::-1], np.argsort(-caption_len, kind='mergesort')
                if 1 in caption_len:
            ...
            #logit_packed = output.contiguous().view(output.size(0) * output.size(1), -1) 
            ...
            #target_packed = target.contiguous().view(target.size(0) * target.size(1))
            #print(logit_packed[:,0] == target_packed)
